...
# Todo:
"""
Be careful : written in python2 !!!!
"""

fact = (1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880)

# Todo : reduced in python 3 ???
...
def sof_digits(n):
    ...
    s = 0
    while n > 0:
        s, n = s + fact[n % 10], n // 10  # for fact, see beginning of the code
    return s
...
# --- number to words ----------------------------------------------------------------------------
# this function copied from stack_overflow user: Developer, Oct 5 '13 at 3:45
def n2words(num, join=True):
    ...
    if num == 0:
        ...
        groups = (numStrLen + 2) // 3  # TODO: check whether if the '//' is ok ?
        numStr = numStr.zfill(groups * 3)
        for i in range(0, groups * 3, 3):
            h, t, u = int(numStr[i]), int(numStr[i + 1]), int(numStr[i + 2])
            g = groups - (i // 3 + 1)  # TODO: idem
            if h >= 1:
