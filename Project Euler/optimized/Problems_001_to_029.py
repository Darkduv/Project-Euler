...
from Tools import timing, numpy_sieve, decompose_tout, fact, sum_digit, binary_search
...
def problem_23():
    ...
    for i in range(2, 28124):
        for j in range(2 * i, 28124, i):
            L[j] += i
    l_abundant = [i for i in range(1, 28124) if L[i] > i]
...
def problem_27():
    ...
    for q in range(-30, 32):
        phi = f(q)
        n = 0
        while binary_search(L, phi(n))[0]:
            n += 1
        if n > maxi:
            maxi = n
            pm = q
    print((1 - 2 * pm) * (pm ** 2 - pm + 41))
